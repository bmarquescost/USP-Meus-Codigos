...
int leitura_de_inteiro() {
    char *linha = NULL;
    linha = leitura_de_linha(stdin);
...
*/
void preenche_lista_dados_csv(AVL *arvore, char *arquivo_csv){
    ...
    for(int i = 0; i < num_linhas; ++i){
        ...
        // Tentando inserir na árvore, caso não consiga ser inserido, significa que o código já existe na árvore
        int inseriu = avl_inserir_site(arvore, site_auxiliar);
        if(inseriu == FALSE) {
            printf("!! Leitura de codigos semelhantes no arquivo CSV !!\n");
            website_deletar(&site_auxiliar);
        }
        else if(inseriu == ERRO) {
            printf("Erro na criação do site\n");
            if(site_auxiliar != NULL) website_deletar(&site_auxiliar);
        }
    }
