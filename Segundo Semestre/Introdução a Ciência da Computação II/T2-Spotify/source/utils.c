...
/*  
    Função readline: 
    Parâmetros: stream - indica de onde será feito a leitura do buffer
...
/*
    Função part_string:
    Parâmetros: string - linha em que será feito a separação
                start - início do índice onde será feito a análise / remoção
                is_end - valor de referência que é útil para casos em que terminamos a string
                         evitando analise de índices posteriores ao final da string
...
/*
    Função read_csv:
    Parâmetros: csv_filename - nome do arquivo em que será feita a leitura
                num_tracks - número de tracks, será atribuído a essa variável recebida por referência
                             o número de tracks lido
